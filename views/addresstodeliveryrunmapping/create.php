...
<body>
<?php /** @var array|null $errors @var array $old @var array $addresstypes @var array $addresses @var array $customers @var array $products @var array $deliveryruns @var array $carriers @var array $flowdirections*/ ?>
<h1>Create Address To Delivery Run Mapping</h1>
    ...
    <?php endif; ?>
<form method="post" action="/addresstodeliveryrunmapping">
    <div>
    ...
    <div>
        ...
        <select name="addressType_id" id="addressType_id" required>
            ...
            <option value="<?= htmlspecialchars($addresstype['id']) ?>"
                <?= (($old['addressType_id'] ?? '') == $addresstype['id']) ? 'selected' : '' ?>>
        ...
        <select name="deliveryRun_id" id="deliveryRun_id" required>
            ...
            <?php foreach ($deliveryruns as $deliveryrun): ?>
                <option value="<?= htmlspecialchars($deliveryrun['id']) ?>"
                    <?= (($old['deliveryRun_id'] ?? '') == $deliveryrun['id']) ? 'selected' : '' ?>>
    ...
    <div>
        ...
        <select name="flowDirection_id" id="flowDirection_id" required>
            ...
            <option value="<?= htmlspecialchars($flowdirection['id']) ?>"
                <?= (($old['flowDirection_id'] ?? '') == $flowdirection['id']) ? 'selected' : '' ?>>
...
<br><br>
<a href="/addresstodeliveryrunmapping">Back to List</a>
</body>
</html>
