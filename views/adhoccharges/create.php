...
<html>
<head>
    <title>Create Adhoc Charge</title>
    <style>
...
<h1>Create Adhoc Charge</h1>
    ...
    <?php endif; ?>
<form method="post" action="/adhoccharges">
    <div>
    ...
    <div>
        ...
        <input id="adhocCharge_name" type="text" name="adhocCharge_name" 
                value="<?= htmlspecialchars($old['adhocCharge_name'] ?? '', ENT_QUOTES) ?>" required>
    </div>
    <div>
        <label for="chargeStructure">Charge Structure:</label>
        <input id="chargeStructure" type="text" name="chargeStructure" 
                value="<?= htmlspecialchars($old['chargeStructure'] ?? '', ENT_QUOTES) ?>" required>
    </div>
    <div>
        <label for="rate">Rate:</label>
        <input id="rate" type="text" name="rate" 
                value="<?= htmlspecialchars($old['rate'] ?? '', ENT_QUOTES) ?>" required>
    </div>
    <div>
        <label for="descriptionTemplate">Description Template:</label>
        <input id="descriptionTemplate" type="text" name="descriptionTemplate" 
                value="<?= htmlspecialchars($old['descriptionTemplate'] ?? '', ENT_QUOTES) ?>" required>
    </div>
    <div>
        <label for="is_enabled">Is Enabled:</label>
        <select id="is_enabled" name="is_enabled" required>
            <option value="">-- Select --</option>
            <option value="1" <?= (($old['is_enabled'] ?? '') == '1' || strtolower($old['is_enabled'] ?? '') == 'yes') ? 'selected' : '' ?>>Yes</option>
            <option value="0" <?= (($old['is_enabled'] ?? '') == '0' || strtolower($old['is_enabled'] ?? '') == 'no') ? 'selected' : '' ?>>No</option>
        </select>
    </div>
    <div>
        <label for="pageVisionOn">Page Vision On:</label>
        <input id="pageVisionOn" type="text" name="pageVisionOn" 
                value="<?= htmlspecialchars($old['pageVisionOn'] ?? '', ENT_QUOTES) ?>" required>
    </div>
    <br>
    <button type="submit">Save</button>
</form>
<br><br>
<a href="/adhoccharges">Back to List</a>
</body>
</html>
